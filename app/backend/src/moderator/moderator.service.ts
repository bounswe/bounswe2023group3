...
export class ModeratorService {
    ...
    public async searchModerators(searchOne:boolean, params): Promise<Moderator | Moderator[]> {
        if (searchOne){
    ...
    async createModerator(createModeratorDto: CreateModeratorDto): Promise<any> {
        ...
        .sendMail({
          ...
          text: 'Here is your verification code: ' + createdModerator.verification_code.toString(), // plaintext body
        ...
        const payload = { sub: createdModerator.id, email: createdModerator.email, userType: 1 };
        return {
      ...
      public async verifyModerator(verifyModeratorDto: VerifyModeratorDto): Promise<void> {
      ...
      public async updatePassword(moderator: Moderator, password: string): Promise<void> {
